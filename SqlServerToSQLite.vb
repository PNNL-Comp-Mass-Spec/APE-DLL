...
Public Class SqlServerToSQLite
    ...
    Public Shared mCreateSeparateTable As String
...
#Region "Private Methods"
        ...
        Else
            For Each stepList As String In workflowStepList.Split(","c)
                ...
                Else
                    ...
                    endStep = wf.Count - 1
                    For i = startStep To endStep
                        If wf.Item(i).WorkflowGroup.Trim = stepList.Trim() Then
    ...
    Private Shared Sub RunWorkflow(ByVal stepsToRun As Hashtable, ByVal workflowTotalSteps As Integer, ByVal Workflow As List(Of clsXMLStepSchema), ByVal sqlitePath As String, ByVal handler As SqlConversionHandler)
        Dim stp, sql, trgTbl, desc, src As String
                ...
                For i = startStep To endStep
                    If stepsToRun.ContainsKey(i) Then
                        ...
                        ElseIf src = "RANGER" Then
                            RunRangerPipeline(Workflow.Item(i).SQL, trgTbl, sqlitePath)
                        ElseIf src = "PLOT" Then
                            RunPlotting(Workflow.Item(i).SQL, trgTbl, sqlitePath, sqliteConnString, handler)
                        Else
        ...
        Try
            ...
            While SQLreader.Read()
                ...
                If plotType.Trim = HISTOGRAMPLOT Then
                    yValue = CDbl(SQLreader(SERIES))
                    xValue = CStr(SQLreader(CATEGORY))
                    lbl(i) = xValue
                    yval(i) = yValue
                    '                    x.SetValue(yValue, i)
                Else
                    xValue = CDbl(SQLreader(SERIESX))
                    yValue = CDbl(SQLreader(SERIESY))
    ...
    Private Shared Sub RunRangerPipeline(pipelineDefinition As String, tblName As String, dbPath As String)
        ...
        ParmPanelRows = pipelineDefinition.Split("@")
        ...
        For i = 0 To ParmPanelRows.Count - 1
            If Not String.IsNullOrEmpty(ParmPanelRows(i).ToString.Trim) Then
                ParmPanelColumns = ParmPanelRows(i).Split("|")
                For j = 0 To ParmPanelColumns.Count - 1
                    ParmPanelValues = ParmPanelColumns(j).Split(":")
